...
const PARAM_SR = [15, 16, 17, 118, 102, 134]; // ORDERED VISUALLY
...
function AllSpeedInner({ recipeId, recipeName, imageUri, onClose, initialParams, pollMs = 2000 }: any, ref: any) {
  ...
  const imgSrc = imageUri ? { uri: imageUri } : require('../assets/allspeed.jpeg'); // 🔁 IMAGE
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}><Text style={[styles.title, dark && { color: '#fff' }]}>
        {recipeName ?? 'All Speed'}
      </Text><Text style={styles.close} onPress={onClose}>Close</Text></View>
      ...
      <ZoomableView minScale={1} maxScale={4} style={{ width: dispW, height: dispH }}>
        <ImageBackground source={imgSrc} style={{ width: dispW, height: dispH }} resizeMode="contain">
          ...
          {PARAM_SR.map(sr => {
            ...
            const left = (pos.x / 100) * dispW; const top = (pos.y / 100) * dispH;
            return (
          ...
          {SERIAL_POS.map(s => {
            const left = (s.x / 100) * dispW, top = (s.y / 100) * dispH;
            return (
...
const styles = StyleSheet.create({
  ...
  paramBox: { position: 'absolute', backgroundColor: 'rgba(255,255,255,0.2)',
